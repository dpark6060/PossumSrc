...
ColumnVector g1motionRV;
ColumnVector g2motionRV;
ColumnVector g3motionRV;
ColumnVector g4motionRV;
...
ColumnVector free(const ColumnVector m,const double time_i,const RowVector tissue,const double phase_i,const double actinttt){
 ...
 // phase_i=phase-rfphase the same logic as the time 
	Matrix W(3,3);                                  // Matrix will hold the exponentials from eqs 4.25 - 4.27
	ColumnVector Q(3);                              // Matrix will hold anything added to the exponentials 
	double e2=exp(-time_i/tissue(2)+actinttt);      // T2 exponential decay at time_i
	double e1=exp(-time_i/tissue(1));               // T1 exponential recovery at time_i
	W <<e2<<0<<0
		<<0<<e2<<0
		<<0<<0<<e1;
	Q=0.0;
	Q(3)=tissue(3)*(1-e1);                          // only eq 4.27 has an added component
	ColumnVector m1(3);                             // Will hold Mx, My, Mz
	m1=W*rot(phase_i,"z")*m+Q;                      // Calculates eq 4.25-4.27 Mx0, My0, Mz0 are calculated based on phase_i, the rotation added by the rf pulse
	return m1;
}
////////////////////////////////////////////////////////////////////////////////
...
double i1(const double gold,const double gnew,const double told,const double tnew){
	...
	double g1,g2;
	g1=0.0;g2=0.0;
	coeff(gold,gnew,told,tnew,g1,g2);               // g1 = Intercept, g2 = Slope
...
double I(const int h,const RowVector& r1,const RowVector& r2, const Matrix& g){
	//Matrix M=rotmat(r2);
	Matrix M=rotmat(r2); // current rotation
	Matrix A=axismat(r1); // control point rotation
	Matrix AM=A*M;
	Matrix A2M=A*AM;
...
void sorter(PMatrix& pulse, const Matrix& motion, const string pulsefile,const int opt_test){//ok
	...
	if (opt_test==1) cout<<"Are you here?"<<endl;
		for (int kk=1;kk<=3;kk++){
		int ll=cc;
		while (iii+ll<=dimnew){
...
//cout<<"new elements:"<<pulse.time(dimp)<<pulse(100,2)<<pulse(100,3)<<pulse(100,4)<<pulse(100,5)<<pulse(100,6)<<pulse(100,7)<<endl;
	...
	Matrix dmat;
	Matrix fmat;
	dmat.ReSize(pulse.Nrows(),1);
	fmat.ReSize((pulse.Ncols()-1)*pulse.Nrows(),1);
	cout<<"dmat size: "<<dmat.Nrows()<<endl;
	cout<<"fmat size: "<<fmat.Nrows()<<endl;
	cout<<"pulse rows: "<<pulse.Nrows()<<endl;
	cout<<"pulse cols: "<<pulse.Ncols()<<endl;
	
	for (int n=0; n<pulse.Nrows(); n++) dmat(n+1,1)=pulse.dmat[n];
	for (int n=0; n<(pulse.Ncols()-1)*pulse.Nrows(); n++) fmat(n+1,1)=pulse.fmat[n];
	
	write_ascii_matrix(dmat,pulsefile+".dmat_mot");
	write_ascii_matrix(fmat,pulsefile+".fmat_mot");
}
...
///////////////////////////////////////////////////////////////////////////
	...
	m(1)=0;
	m(2)=0;
	m(3)=tissue(3);     // Lazily saying that Mo=rho, instead of 6.12 
...
///////////////////////////////////////////////////////////////////////////
	...
	g1=0.0; g2=0.0; g3=0.0; grf1=0.0; grf2=0.0; grf3=0.0; //Haven't Done Gradients Yet
	Matrix coord(3,nreadp); // - nreadp = number of readoutpoints !!!  I believe this stores xyz coordinates?

	if ( v==1 ) // If this if the first voxel
	{
				// these are initialized on the first voxel, and its entire time series is calculated.
		g1static = new double[numpoints];
		g2static = new double[numpoints];
		g3static = new double[numpoints];

		g1motionRV.ReSize(numpoints);
		g2motionRV.ReSize(numpoints);
		g3motionRV.ReSize(numpoints);
		g4motionRV.ReSize(numpoints);
		...
		glo_cx=gammabar*xdim;	//(Hz*m/T) - gamma bar * the dimensions of a voxel, kind of like a frequency/vox thing?
...
///////////////////////////////////////////////////////////////////////////
	...
	{ 
		double tnew = H.time(step);       // H: (1)=time in s,(2)=rf angle,(3)=rf frequency bandwidth df(Hz),(4)=rf center frequency fc(Hz),(5)=readout 1/0 (6)=x gradient(T/m),(7)=y gradient(T/m),(8)=z gradient(T/m)
		...
		if (v==1)  // If this is the first voxel in the read, initialize stuff
		{
						// these values are SET for all other voxels after the first initialization...I think...
			double gxold = H(step-1,6);  // Old X gradient
			double gyold = H(step-1,7);  // Old Y gradient
			double gzold = H(step-1,8);  // Old Z gradient (from previous time step)

			g1 += i1(gxold,gxnew,told,tnew); //i1 - integral function - so g1,2,3 are tracking the cumulative gradient applied I blieve... T/m
			if (gynew!=0 || gyold!=0)	g2+=i1(gyold,gynew,told, tnew);
			if (gznew!=0 || gzold!=0)	g3+=i1(gzold,gznew,told, tnew);

			g1static[step-2] = g1; // Setting a static x field to current field integral for x,y,z
			g2static[step-2] = g2;
			g3static[step-2] = g3;
		}
		else
		{
			g1 = g1static[step-2]; // if it's not the first voxel, we just use the old static field
			g2 = g2static[step-2];
			g3 = g3static[step-2];
						// g1static never seems to get updated, how can this be?
				}

		double gg1 = g1 - grf1;     // gg1 is the change in gradient integral... from the previous step.
...
//		double phase = 0;
		double phase = gama*gg1*x + gama*gg2*y + gama*gg3*z+gama*b0*tt + gama*chshift*tt;  // This is more like a K space magnitude, eq 9.49/50 and 10.1 - 10.3

		if (rfangle != 0)
		{
			excitation = 0;
			double df = H(step,3);  //frequency width - slice thickness(m) times the gradient (T/m) Times gammabar (Hz/T) (in m*Hz/T*T/m=Hz),
			double fc = H(step,4);  // RF center frequency (Hz?)
			double f = gammabar*(gxnew*x+gynew*y+gznew*z+b0+chshift); //Resonsnace frequency at point x y z (Hz?)

			if (opt_test==1)	cout<<"gxnew="<<gxnew<<"; x="<<x<<"; gynew="<<gynew<<"; y="<<y<<"; gznew="<<gznew<<"; z="<<z<<"; fc="<<fc<<"df="<<df<<". For voxel v="<<v<<": f="<<f<<endl;
			rftest = 0;
			double fval = (f - fc)/df;  // The resonence minus the center frequency divided by the bandwidth.  I guess what fraction of the total bandwidth are we off from the RF pulse's center frequency?  Unitless?
			double off = (fval - dslcp_first)/dslcp;  // dslcp is the frequency in the the slice profile, dslcp_first is frequency offset from the center of the RF bandwidth.
																											// I guess it's the frequency point to start at?  adjusting for gradients? Maybe it determines where this x/y/z coordinate falls in the slice profile?
...
//			if (opt_test==1)	cout<<"fc="<<fc<<"; f"<<f<<"; df"<<df<<"; fval"<<fval<<"; dslcp_first"<<dslcp_first<<"; dslcp"<<dslcp<<"; off="<<off<<"nf"<<nf<<endl;
			if (nf >= 0 && nf <= (Nslc-2))
			{
				off -= nf;  // off should be zero or the fractional part of off now
				double ts = table_slcprof[nf];  // actual slice RF value - between 0 and 1
				double sx = (table_slcprof[nf+1]-ts)*off + ts;  // change in slice RF value * the frequency offset plus the rf slice value
																																// I think this is trying to account for the innacuracy from (int) off
				double rfangle_f = sx*rfangle*RFtrans;//RFtrans are values 0 to 1 to derscribe the inhomogeneity f the receive RF field. 1 is for perfectly homog;  0 to 1 modulation of RF flip angle
...
//due to crushers or any gradient induced dephasing over the voxel a new initial magnetisation is introduced which is the average of the magnetisations over the voxel  
										// b0x is field inhomogeneity, which we don't have, so just glo_cx*gg1)
										// gg1... see below...
										//glo_cx Hz*m/T*T/m = Hz
										// This is extracting the rf frequency of the voxel
					double xvalrf = fabs(glo_cx*(gg1 + b0x*tt)); //glo_cx=gammabar*xdim;	//(Hz*m/T) Frequency span of x direction  * change in gradient + gradient inhomogeneity (T/m)*time
					...
					m00 = sqrt( m(1)*m(1)+m(2)*m(2) );
										// Magnetic vectors are rotated according to that frequency
					if (opt_test==1 && readstep%4096==0 && v==1 )	cout<<"Projection of the magnetisation vector into the xy plane after flipping is "<<m00<<endl;
										// Set the trf - time of RF pulse to tnew
					trf = tnew;

										
										
										// Updating grf1 to g1 ... grf1 starts at zero, and is updated to g1.
										// In the next loop, g1 is set to g1static[step-2], which is the
										// total gradient integral calculated during the first voxel loop.
										//  Then, gg1 is calculated as g1 - grf1
										// g1 = gradient integral at current step
										// grf1 = gradient integral at previous step
										// gg1 = current integral - previous integral = integral of current gradient from last step
										// You can't quite just say that it's the value of the gradient, because step times might be different.
										// if step times were "1", it would be just the current gradient value.
...
void voxel2(const double x,const double y,const double z, 
		...
		cout<<"Creating Matrix corrd(3,"<<nreadp<<")"<<endl;
		Matrix coord(3,nreadp);
		cout<<"Created"<<endl<<endl;
		if (v==1) {
			cout<<"v==1, resize g1motion"<<endl;
		...
		for (int step=2;step<=numpoints;step++){
			...
			if (v==1){
				...
				trnew2=H(step,10);//
				trnew3=H(step,11);//
					g(1,1)+=i1new(gxold,gxnew,told, tnew);
...
void voxel3(const double x,const double y,const double z, 
	...
	if (v==1) {
		...
		cout<<"Resizing motionRV"<<endl;
		g1motionRV.ReSize(numpoints-1);
		g2motionRV.ReSize(numpoints-1);
		g3motionRV.ReSize(numpoints-1);
		g4motionRV.ReSize(numpoints-1);
		cout<<"Done"<<endl;
