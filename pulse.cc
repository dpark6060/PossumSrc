...
//tejas-end
 ...
 double TG=TF+2*rt+tcrush;//100rt is how much is needed to make ggx ggy and ggz become 100 times larger which will reduce the signal 100 times -----  still in process of testing dec 20
  ...
  cout<<"maxG:  "<<maxG<<endl;
  cout<<"rt:  "<<rt<<endl;
  cout<<"TRslc:  "<<TRslc<<endl;
  cout<<""<<endl;
  cout<<"df:  "<<df<<endl;
  cout<<"rft:  "<<rft<<endl;
  cout<<"dtz:  "<<dtz<<endl;
  cout<<"dtz1:  "<<dtz1<<endl;
  cout<<"dtz11:  "<<dtz11<<endl;
  cout<<""<<endl;
...
////////////////////////////
 ...
 for (int a=1;a<=n;a++){
   ...
   for (int b=1;b<=ns;b++){
     ...
     ////////////////readout
	 cout<<"kx at start of redout:  "<<kx<<endl;
	 cout<<"---------"<<endl;
	 cout<<"T at start of redout for volume "<<a<<" : "<<t<<endl;
	 cout<<"---------"<<endl;
	 ...
	 //  cout<<"The first point in the k-space for the VolNum "<<a<<" and the SlcNum "<<b<<" is (kx,ky)="<<"("<<kx*gammabar<<","<<ky*gammabar<<")"<<endl; 
         ...
         coord(2,readstep)=ky*gammabar;
	   	...
	   	cout<<"kx at start of Line "<<c<<" :  "<<kx<<endl;
		//cout<<"---------"<<endl;
  ...
  write_ascii_matrix(coord,opt_pulse.value()+"_kcoord_ascii");
 if (opt_kcoord.value()) write_binary_matrix(coord,opt_pulse.value()+"_kcoord");
 cout<<"STEP: "<<step<<endl;
...
//tejas-end
 ...
 double TA=rft/2+dtz+dtz1;//when all rf stuff ends
 double df=ddz*gammabar*Gz;//frequency width - slice thickness(m) times the gradient (T/m) Times gammabar (Hz/T) (in m*Hz/T*T/m=Hz),
 //cout<<"frequency width     "<<df<<endl;
 double t=0;//time (in ms)
 //double offset=0;//for now (in m)
 int step=1;//row number in the matrix
...
////////////////////////////
 ...
 Matrix coord(2,nreadp); //matrix for the kspace coordinates, 1st raw for kx and 2nd for ky
	...
	for (int a=1;a<=n;a++) // Number of volumes
	{
		for (int b=1;b<=ns;b++) // Number of slices per volume
		{
			for (int c=1;c<=resY;c++)  // Current y voxel/kspace
			{
				kx=0;ky=0;
			       ////////////////////////////////////////
...
//write_binary_matrix(M,"PMatrix_value");
 ...
 if (opt_kcoord.value())
 {
	write_ascii_matrix(coord,opt_pulse.value()+"_kcoord" );
 }
...
int compute_volume(){
 ...
 if (bottom+top+1>resY) {
  ...
  if (opt_seq.value()=="epi") {
	...
	//(1)=time in s
	//(2)=rf angle
	//(3)=rf frequency bandwidth df(Hz)
	//(4)=rf center frequency fc(Hz)
	//(5)=readout 1/0
	//(6)=x gradient(T/m)
	//(7)=y gradient(T/m)
	//(8)=z gradient(T/m)
  ...
  write_ascii_matrix(dmat,opt_pulse.value()+".dmat");
  write_ascii_matrix(fmat,opt_pulse.value()+".fmat");
  //write_ascii_matrix(pulse(:,1),opt_pulse.value()+".time");
	//write_ascii_matrix(pulse(:,4),opt_pulse.value()+".rf_centerf");
	//write_ascii_matrix(pulse(:,5),opt_pulse.value()+".readout");
	//write_ascii_matrix(pulse(:,6),opt_pulse.value()+".xgrad");
	//write_ascii_matrix(pulse(:,7),opt_pulse.value()+".ygrad");
	//write_ascii_matrix(pulse(:,8),opt_pulse.value()+".zgrad");
