...
//end
  ...
  volume<double> RFrec(Nx,Ny,Nz);//signal=signal*RFrec # RF inhomogeneity is not reall used I think.  
    ...
    for (int px=0;px<Nx;px++){
      for (int py=0;py<Ny;py++){
        for (int pz=0;pz<Nz;pz++){
          RFrec(px,py,pz)=1;		// set for giant volume of ones
	}
      }
    }
  }
  ...
  if (opt_RFtrans.set())  read_volumeROI(RFtrans,opt_RFtrans.value(),myid,0,0,Nxx,-1,-1,numprocs,1,1);
    ...
    for (int px=0;px<Nx;px++){
      for (int py=0;py<Ny;py++){
        for (int pz=0;pz<Nz;pz++){
          RFtrans(px,py,pz)=1;  // 3d volume of transmission homogeneity.  usually 1
	}
      }
    }
  }
		...
		for (register int yy=ystart;yy<yend;yy++){
		  ...
		  for (register int xx=xstart;xx<xend;xx++){
		    ...
		    if (phantom(xx,yy,zz,tt)!=0){
		      ...
		      double den=phantom(xx,yy,zz,tt)*RFrec(xx,yy,zz)*cxyz; // the phantom timtes RF field inhomegeniety, times the total number of voxels (nx*ny*nz).  Phantom is input object
		      if (opt_activation.set()) {
		        for (int n=0;n<=Nact-1;n++){
